...
std::string TestCase::save_test_case(TestCase tc){
	std::string file_name_player_image = "player_test_case_" + int_to_string(ms_instance_number) + ".jpg";
	bool player_save = m_player_image.save(file_name_player_image);
	if (!player_save){
	...
	std::string file_name_template_A = "template_A_test_case_" + int_to_string(ms_instance_number) + ".jpg";
	std::string file_name_template_B = "template_B_test_case_" + int_to_string(ms_instance_number) + ".jpg";
	std::string file_name_template_REFEREE = "template_REFEREE_test_case_" + int_to_string(ms_instance_number) + ".jpg";

	bool template_A_save = m_templates[IDX_TEAM_A].save(file_name_template_A);
	bool template_B_save = m_templates[IDX_TEAM_B].save(file_name_template_B);
	bool template_REFEREE_save = m_templates[IDX_TEAM_REFEREE].save(file_name_template_REFEREE);
...
TestCase TestCase::decode_from_string(std::string str){
	...
	Template t_A = Template(template_A_image, TEAM_A);
	Template t_B = Template(template_B_image, TEAM_B);
	Template t_REFEREE = Template(template_REFEREE_image, TEAM_REFEREE);
	std::vector<Template> templates;
	templates.push_back(t_A);
	templates.push_back(t_B);
	templates.push_back(t_REFEREE);

	return TestCase(player_image, expct, templates);
}
